...
export async function showConfig(workDir = process.cwd()): Promise<void> {
  if (!isInitialized(workDir)) {
    logWarn("No config found. Run 'morty --init' first.");
    return;
  }
  ...
  console.log("");
  console.log(`${pc.bold("Morty Configuration")} (${configPath})`);
  console.log("");
  ...
  if (config.project.description)
    console.log(`  About:     ${config.project.description}`);
  console.log("");
  ...
  if (config.rules.length > 0) {
    ...
    console.log(`  ${pc.dim('(none - add with: morty --add-rule "...")')}`);
  }
  console.log("");
...
export async function addRule(
  ...
  if (!isInitialized(workDir)) {
    logError("No config found. Run 'morty --init' first.");
    return;
  }
